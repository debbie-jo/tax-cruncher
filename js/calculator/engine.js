...
function runEngine(state) {
  ...
  let stream1;
  if (ruleYear1 >= 2025) {
    // year1 ≥ 2025: 신규정 — 직전년도도 동일 규칙으로 재계산 (입력값은 구규칙 기준이므로 불일치)
    const prevStats = calcStats(employees, year - 1, ruleYear1);
    const py1_eff = { total: prevStats.total, youth: prevStats.youth, nonYouth: prevStats.nonYouth };
    stream1 = calcTaxCredit2025(currentStats, py1_eff, companyType, region, year, year);
    stream1.prevTotal    = py1_eff.total;
    stream1.prevYouth    = py1_eff.youth;
    stream1.prevNonYouth = py1_eff.nonYouth;
  } else {
    // year1 ≤ 2024: 종전 규정 — pre-2025 단가
  ...
  const y1_s2 = year - 1;
  if (py1 && y1_s2 <= 2025) {
    // year1=2025 이하 → 구규칙(M/12) 적용
    const cs2 = calcStats(employees, year, y1_s2);
  ...
  const y1_s3 = year - 2;
  if (py2 && y1_s3 < 2025) {
    const cs3 = calcStats(employees, year, y1_s3);
